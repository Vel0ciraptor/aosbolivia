...
import {
  Wrench, Search, CheckCircle2, XCircle, Clock, AlertCircle,
  RefreshCw, ChevronRight, Phone, MapPin,
} from 'lucide-react';
...
interface WorkshopRow {
  ...
  _count: { services: number; quotes: number };
}
...
export default function AdminWorkshopsPage() {
  ...
  const filtered = useMemo(() => {
    return workshops.filter((w) => {
      if (statusFilter !== 'ALL' && w.estado !== statusFilter) return false;
      if (search.trim()) {
        const q = search.toLowerCase();
        const haystack = `${w.nombre} ${w.user.name} ${w.direccion} ${w.descripcion ?? ''}`.toLowerCase();
  ...
  const stats = useMemo(() => ({
    ...
    active: workshops.filter((w) => w.estado === 'ACTIVE').length,
  }), [workshops]);

  const handleToggleStatus = async (w: WorkshopRow) => {
    const next = w.estado === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE';
    if (!confirm(`¿Cambiar estado de "${w.nombre}" a ${next}?`)) return;
    try {
      setActionId(w.id);
      await api.patch(`/admin/workshops/${w.id}/status`, { estado: next });
      await fetchData();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="p-12 bg-zinc-900/30 border border-zinc-800/80 border-dashed rounded-3xl text-center">
          ...
          <p className="text-zinc-500 text-sm mt-1">No se encontraron talleres con esos filtros.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((w) => {
            const statusMeta = STATUS_META[w.estado] || STATUS_META.ACTIVE;
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-bold text-zinc-200 text-sm truncate">{w.nombre}</p>
                    ...
                    <p className="text-[11px] text-zinc-500 truncate">{w.user.name}</p>
                    <p className="text-[10px] text-zinc-600 truncate mt-0.5 flex items-center gap-1">
                      <MapPin className="w-2.5 h-2.5" /> {w.direccion}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-zinc-800/60 text-[11px] text-zinc-400">
                  <div>{w._count.services} servicios</div>
                  <div>{w._count.quotes} cotizaciones</div>
                </div>
                <div className="flex items-center gap-1.5 pt-1">
                  <Link
                    href={`/dashboard/admin/users/${w.user.id}`}
