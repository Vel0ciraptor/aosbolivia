...
export default function MyQuotesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const res = await api.get(`/quotes/provider/${providerId}`);
          ...
          {quotes.length === 0 ? (
            <>
              ...
              <p className="text-zinc-500 text-sm mt-1 max-w-sm mx-auto">
                Explora las solicitudes de repuestos y envía tu primera cotización.
              </p>
              <Link
        ...
        <div className="space-y-3">
          {filtered.map((q) => {
            ...
            return (
              <Link
                key={q.id}
                href={`/dashboard/provider/requests/${q.request.id}`}
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0 space-y-3">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-zinc-100 group-hover:text-indigo-300 transition-colors flex items-center gap-2">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <ArrowRight className="w-4 h-4 text-zinc-500 group-hover:text-indigo-400 transition-colors ml-auto mt-1" />
                  </div>
                </div>
              </Link>
            );
