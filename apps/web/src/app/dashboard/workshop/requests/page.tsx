    ...
    async function load() {
      try {
        const [reqRes, quotesRes] = await Promise.all([
          ...
          api.get(`/quotes/workshop/${workshopId}`),
        ]);
        if (cancelled) return;
        const all = (reqRes.data || []).filter((r: RequestItem) => r.categoria === 'TALLER');
        setAllRequests(all);
  ...
  if (workshopError || !workshop) {
    return (
      <div className="p-8 bg-red-950/20 border border-red-800/40 rounded-2xl text-center">
        ...
        <p className="text-sm text-zinc-400 mt-1">{workshopError || 'Verifica que tu cuenta esté vinculada a un taller activo.'}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-400">
          Estas son todas las solicitudes de clientes que necesitan servicios mecánicos.
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="p-12 bg-zinc-900/30 border border-zinc-800/80 border-dashed rounded-3xl text-center">
          ...
          {allRequests.length === 0 ? (
            <>
              <h3 className="font-bold text-zinc-300 text-base">No hay solicitudes de taller</h3>
              <p className="text-zinc-500 text-sm mt-1 max-w-sm mx-auto">
                Cuando los clientes publiquen solicitudes mecánicas, aparecerán aquí para que puedas cotizar.
              </p>
            </>
          ) : (
            <>
